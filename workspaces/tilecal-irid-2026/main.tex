...
\begin{columns}
  ...
  \column{0.333}
    ...
    \block{The ATLAS Hadronic Tile Calorimeter (TileCal)}{
      ...
      \end{minipage}
        ...
        {\small Figure 1: ATLAS Tile Calorimeter (left) and a wedge-shaped module slice (right) \cite{ATLAS2008}.}
    }
    ...
    \block{Radiation Criteria}{
      ...
      \begin{tabular}{|l|c|c|c|}\hline
        ...
        \hline
        PMT Divider -- Barrel & 17.6 & 2.0$\times$10$^1$$^2$ & 1.9$\times$10$^1$$^1$ \\
        FENICS -- Barrel & 10.6 & 1.6$\times$10$^1$$^2$ & 1.8$\times$10$^1$$^1$ \\
        MB -- Barrel & 10.0 & 1.4$\times$10$^1$$^2$ & 9.2$\times$10$^1$$^0$ \\
        LVPS -- Barrel & 53.6 & 3.5$\times$10$^1$$^2$ & 5.3$\times$10$^1$$^1$ \\
        DB -- Endcap & 6.8 & 9.7$\times$10$^1$$^1$ & 4.9$\times$10$^1$$^0$ \\
      \hline
      \end{tabular}
      }
      {\small Table 1: Simulated worst-case doses for 4000 fb$^-$$^1$ (no safety factors), only higher doses from Barrel/Endcap are shown.}
    }
  ...
  \column{0.333}
    ...
    \block{LVPS Design \& Component Radiation Testing}{
      \begin{itemize}\setlength{\itemsep}{0.3em}
        ...
        \item LT1681 Controller \& LT3080 Regulator: TID tests at 502 Gy/h showed minor drifts ($\le$5\%) in output voltages, within acceptable limits.
      ...
      \end{minipage}
        ...
        {\small Figure 2: SIHFS9N60A MOSFET characterization at V\_DS = 1 V after 200 Gy of TID (left) and after NIEL tests (right) \cite{Moayedi2026}.}
    }
  ...
  \column{0.333}
    ...
    \block{Daughterboard (DB) Architecture}{
      ...
      \end{center}
      {\small Figure 3: The TileCal Daughterboard redundancy architecture and data paths \cite{ValdesSanturio2023}.}
    }

    % block id: blk_db_qualification  (column 3, order 1)
    \block{DB Radiation Qualification \& Tolerances}{
      \begin{itemize}\setlength{\itemsep}{0.3em}
        \item The DB requires 108 Gy TID and 13.16$\times$10$^1$$^2$ n/cm$^2$ NIEL qualification limits (including safety factors).
        \item Kintex Ultrascale (KU) FPGAs (primary data processors) and ProASIC FPGAs (remote reconfiguration buffers) successfully withstood over 108 Gy and 14$\times$10$^1$$^2$ n/cm$^2$ during tests.
        \item KU+ FPGAs were disqualified for exhibiting SEL; selected KU FPGAs showed no SEL up to 3.2$\times$10$^1$$^1$ p/cm$^2$.
        \item Measured SEU rates for the KU FPGA were 116 SEU/(10$^9$ p/cm$^2$); \textasciitilde{}5498 SEUs per year, heavily mitigated by Xilinx SEM.
      \end{itemize}

      \begin{center}
        \includegraphics[width=1.0\linewidth]{assets/fig_db6_tid_test.png}
      \end{center}
      {\small Figure 4: Monitored KU FPGA temperatures and currents during 54 MeV proton beam TID testing. Drops correspond to active SEU corrections by the Xilinx SEM and resets applied during uncorrectable SEUs \cite{ValdesSanturio2023}.}
    }
