...
| A-3 | Medium | Citation-audit `confidence` (medium/low fallback matches via OpenAlex/Tavily in `verifySingleCitation`) is computed but discarded before it ever reaches a human: `auditThesisCitations()` counts any `verification.found===true` as "verified" regardless of confidence; `thesis-review/route.ts` filters the audit down to only `!found \|\| iso690Issues.length>0` before it goes into the LLM prompt (`citationAuditSummary`) and the persisted `citationIssues` field rendered by `CitationIssuesPanel`. The richer per-result object (with `confidence`) *is* separately captured in `phdEnrichment.citationAudit` (PhD-opponent path, `review-engine.ts`) and persisted to DB via `review-serializer.ts`, but no component under `components/` reads `.citationAudit` and `review-composer.ts` never touches it either — confirmed via full-repo search, 0 hits outside tests/type-defs. Net effect: a fabricated/non-existent citation that Semantic Scholar correctly reports `not_found` for, but that then loosely matches an unrelated OpenAlex or Tavily search result, is silently relabeled "verified" with **no confidence signal reaching the reviewer, the LLM grounding context, the DB-persisted issues list, or any export** — it reads as clean everywhere the app surfaces it. | 🔴 Open — found 2026-09-03 |
