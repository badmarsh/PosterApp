...
new_code = '''import type { Card, Project, OutputConfig } from "@/lib/poster-types"
...
function generateFigures(card: Card, workspaceId = ""): string {
  ...
  if (figs.length >= 2) {
    ...
    const captionA = rawCapA ? `\\vspace{0.4ex}\\par{\\small\\textit{${parseMarkdownToLatex(rawCapA)}}}` : ""
    const captionB = rawCapB ? `\\vspace{0.4ex}\\par{\\small\\textit{${parseMarkdownToLatex(rawCapB)}}}` : ""
    return [
...
function parseMetricItems(content: string): { intro: string; items: MetricItem[] } {
  ...
  for (const rawLine of lines) {
    ...
    // Detect bold hero stat patterns:
    // e.g. - **28.4 BLEU** EN-DE translation (+2.0 over SOTA)
    // or **28.4** | BLEU EN-DE | +2.0 over SOTA
...
with open('lib/latex/generator-poster.ts', 'w', encoding='utf-8') as f:
    f.write(new_code)

print("Successfully written lib/latex/generator-poster.ts")
