...
describe("Generator Paper", () => {
  ...
  it("produces valid JINST proceedings document", () => {
    const prj = makeProject("jinst-proceedings")
    const res = new StandardPaperGenerator("jinst-proceedings").generateDocument(prj, prj.outputs[0], "prj_1")
    expect(res).toContain("\\documentclass[a4paper,11pt]{article}")
    expect(res).toContain("\\usepackage{jinstpub}")
    expect(res).toContain("\\proceeding{Test Venue}")
    expect(res).toContain("\\title{\\boldmath Test Paper}")
    expect(res).toContain("\\author{John Doe}")
    expect(res).toContain("\\begin{document}")
    expect(res).toContain("\\flushbottom")
    expect(res).toContain("\\end{document}")
  })

  it("produces valid PoS proceedings document", () => {
    const prj = makeProject("pos-proceedings")
    const res = new StandardPaperGenerator("pos-proceedings").generateDocument(prj, prj.outputs[0], "prj_1")
    expect(res).toContain("\\documentclass[a4paper,11pt]{article}")
    expect(res).toContain("\\usepackage{pos}")
    expect(res).toContain("\\FullConference{Test Venue}")
    expect(res).toContain("\\title{Test Paper}")
    expect(res).toContain("\\author{John Doe}")
    expect(res).toContain("\\begin{document}")
    expect(res).toContain("\\end{document}")
