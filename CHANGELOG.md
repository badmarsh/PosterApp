...
## [Unreleased]
### Playwright E2E for the new surfaces (2026-09-17 session, follow-up)
- **New `tests/error-lens.spec.ts`** — 5 browser tests driving the real editor UI against the in-memory demo workspace with only the HTTP boundary stubbed (`page.route`): compile error-lens triage + jump-to-card, raw-log toggle (new `data-testid="compile-raw-log"` on the raw `<pre>`), quick-fix apply, height-meter overflow, and academic-search credibility/retracted pills. No database, LaTeX compiler or external API required — green in ~12 s.
- Known defect surfaced (documented in `docs/audit-2026-09-17.md`, unfixed): agent panel crashes rendering a chat-provider failure after an auto-appended compile log.
